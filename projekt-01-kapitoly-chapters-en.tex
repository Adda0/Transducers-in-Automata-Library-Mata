...
We remove duplicated \nfts in each benchmark which in total produces $1749$ instances.

\paragraph{Application of a literal benchmark.}
The appliation of a literal benchmark consists of pairs ($u$, $\ft$) where $u$ is a string literal and $\ft$ is a $2$-tape \nft for replace operations found in SMT-LIB formulae solved by \noodler.
...
We remove duplicated pairs ($u$, $\ft$) in each SMT-LIB benchmark which leaves us in total with $2027$ instances.
...
\begin{table}[ht]
  ...
  \caption{
    Table comparing performance of \mata and \mona on an operation language application on both benchmarks where the language is forward applied ($\id{\langof{\pi}} \composePipe \ft$), simulating the forward image propagation.
    The shown times are in milliseconds.
    The table shows the tool and which projection was performed used (tool name and the tape being projected out); number of timeouts; and runtimes: minimal, maximal, mean, quantile 0.25, median (quantile 0.50), quantile 0.75, standard deviation.
  }
...
\begin{figure}[ht]
    ...
    \caption{
      Scatter plots for comparing the performance of \mata and \mona on application operations on both benchmarks.
    }
