...
$\aut^{\text{short}}_{\pi}$ now matches the shortest words in the language of $\aut_{\pi}$, but does not allow skipping over additional $[$ encountered during reading the match.
Such $[$ need to be skipped since $[$ inside the shortest left-most match represent beginnings of potential matches further to the right than the left-most match.
We need to therefore add a self loop to every state in $\aut^{\text{short}}_{\pi}$ with transition symbol $[$.

Now the final modification is to keep the next begin marker for the next shortest left-most match. $\aut^{\text{short}'}_{\pi}$ can be constructed given $\Sigma^*_[ = \Sigma^* \cup \{ [ \}$ as $\aut^{\text{short}'}_{\pi} = \aut^{\text{short}}_{\pi} \cap \overline{\Sigma^*_[ \concat \{ [ \}}$.

$\aut^{\text{short}'}_{\pi}$ can be converted into $\ft^{\text{short}'}_{\pi}$ where the existing transitions are the input tape transitions, and the output symbols are all $\epsilon$ (\nft only reading the input word---the match---and consuming the whole match including $[$ inside the match).
...
Here we do not return to the beginning after the first replace, but move to a new state with just reads the rest of the word unmodified while removing the additional $[$.

\subsubsection{Whole Regex Reluctant Replace \nft}

We have constructed $\ftBeginMarker$ to insert non-deterministically begin markers into the input word and constructed $\ftRegexRelucReplaceAll$ (or $\ftRegexRelucReplaceSingle$) to perform the replacement upon encountering matches prepended with $[$.
...
% The general idea is that we utilize these \nfts to process $x$ from left to right, and when encountering the first $[$, start reading the shortest matching pattern enclosed in $[ v ]$ for $v \in \pi$, then we replace the read pattern with the $y$ and go back to the beginning of the procedure to look for the next shortest match.

% ,gets applied on the begin marker \nft which produces the input word annotated with begin markers. Now the word is applied to the second \nft.
% The second \nft encodes an end marker

% TODO: Modelling reluctant regular replacements.

\begin{itemize}
    \item \dfts for finite replacement
    \item differences for single literal, single symbol
\end{itemize}
