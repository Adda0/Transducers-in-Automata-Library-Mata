...
For the given problem $ u = x^{+}_{\pi \rightarrow y}$ with $\aut_x$ and $\aut_u$ being the regular languages assigned to variables $x$ and $u$, we can compute the language of the forward image as $\texttt{out}(\idNft{\aut_x} \compose \ft_{x^+_{\pi \rightarrow y}})$;
the backward image as $\texttt{in}( \ft_{x^+_{\pi \rightarrow y}} \compose \idNft{\aut_u})$

% TODO: $x$ is a concatenation of langauges with special epsilon symbols used in noodlification.
