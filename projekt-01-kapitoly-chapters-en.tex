...
\begin{example}
  ...
  Figure~\ref{fig:literal_reluctant_replace} shows both variations for replace all and replace single \nfts for $x^{+}_{cc \rightarrow a}$.

  Note that thanks to our approach, $\ftLiteralRelucAll$ and $\ftLiteralRelucSingle$ after composition maintain the same structure, only marker symbol $\$$ is replaced with $\epsilon$.
...
However, adding this support for \noodler for its stabilization decision procedure is is a challenging task with interesting impactful qualities.
