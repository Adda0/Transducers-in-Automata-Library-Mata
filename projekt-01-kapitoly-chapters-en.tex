...
We could start constructing the \emph{begin marker} \nft from $\ftEndMarker$ by reversing all transitions in $\ftEndMarker$.
...
Henceforth, we have introduce another source of non-determinism, which gives us the "look-ahead" power for marking beginnings for only those inputs that will later on truly match in $\pi$.
