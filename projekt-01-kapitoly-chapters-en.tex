...
For the implementation of the current interface and algorithms for \nfts, we used the existing implementation for a simple prototype BDD representation using \nfas with levels, implemented previously in \mata, and expanded upon that. We further collaborated with our colleagues to define and implement the same interface for \nfts and future proper implementation of BDDs, based on \nfts.
As a result of discussions and collaboration, we add support for \nfts (this work, utilizing the existing implementations for application, projection and composition) and plan adding support for BDDs in the future, we agreed upon a common interface, where BDDs inherit operations from \nfts where the cores of the \nft- and BDD-specific operations from Section~\ref{sec:Algorithms} (composition, projection, application) are adopted and extended versions of the existing implementations for BDD operations.
